...
class MiddlewareTestingTest {
    @Test
    fun `forwardedActions returns the sequence a middleware emits`() =
        runTest {
    ...
    @Test
    fun `lifted middleware re-embeds forwarded actions`() =
        runTest {
            ...
            assertEquals(
                listOf(AppAction.Counter(CounterAction.LoadStarted), AppAction.Counter(CounterAction.LoadSucceeded(42))),
                forwarded,
            )
    ...
    @Test
    fun `keyed middleware addresses the entry and passes unknown keys through`() =
        runTest {
    ...
    @Test
    fun `optional middleware passes through on null state`() =
        runTest {
            ...
            assertEquals(
                listOf(CounterAction.LoadStarted, CounterAction.LoadSucceeded(42)),
                optional.forwardedActions(CounterState(), CounterAction.LoadRequested),
            )
        }
}
