...
class MiddlewareCombinatorsTest {
    ...
    private val logging = Middleware<CounterState, CounterAction> { getState, action, next ->
        next(CounterAction.Log("count=${getState().count}"))
        next(action)
    }

    private fun CoroutineScope.store(middleware: Middleware<AppState, AppAction>, initial: AppState = AppState()) = Store(
    ...
    @Test
    fun `lifted runs the child middleware with the lowered state and re-embeds forwarded actions`() = runTest {
    ...
    @Test
    fun `lifted passes non-matching actions through`() = runTest {
    ...
    @Test
    fun `optional passes through while the state is null and runs once present`() = runTest {
        var seen = 0
        val counting = Middleware<CounterState, CounterAction> { _, action, next -> seen++; next(action) }
    ...
    @Test
    fun `keyed addresses the entry carried by the action and passes unknown keys through`() = runTest {
    ...
    @Test
    fun `offset addresses the element carried by the action and passes out-of-range indices through`() = runTest {
        val store = store(logging.offset(listLens, indexedPrism), initial = AppState(list = listOf(CounterState(), CounterState(count = 8))))
