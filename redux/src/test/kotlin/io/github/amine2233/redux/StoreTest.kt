...
class StoreTest {
    ...
    @Test
    fun `dispatchSuspend applies the reducer`() = runTest {
    ...
    @Test
    fun `dispatch runs on the store scope and updates state`() = runTest {
        val store = Store(CounterState(), counterReducer, scope = this)
    ...
    @Test
    fun `middlewares run in order and chain through next`() = runTest {
        val order = mutableListOf<String>()
        val first = Middleware<CounterState, CounterAction> { _, action, next ->
            order += "first"
            next(action)
        }
        val second = Middleware<CounterState, CounterAction> { _, action, next ->
            order += "second"
            next(action)
        }
    ...
    @Test
    fun `middleware can forward additional actions`() = runTest {
        val logging = Middleware<CounterState, CounterAction> { _, action, next ->
    ...
    @Test
    fun `middleware can swallow an action`() = runTest {
    ...
    @Test
    fun `getState after next returns the reduced state`() = runTest {
        var seen = -1
    ...
    @Test
    fun `concurrent dispatches do not lose updates`() = runTest {
        val store = Store(CounterState(), counterReducer, scope = this)
