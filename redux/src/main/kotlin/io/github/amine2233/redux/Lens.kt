...
public class Lens<T, V>(
    ...
    public fun modify(target: T, transform: (V) -> V): T = set(target, transform(get(target)))

    public infix fun <SubV> then(other: Lens<V, SubV>): Lens<T, SubV> = Lens(
        get = { target -> other.get(get(target)) },
        set = { target, subVal -> modify(target) { other.set(it, subVal) } }
    )

    public infix fun <SubV> then(optional: Optional<V, SubV>): Optional<T, SubV> = Optional(
